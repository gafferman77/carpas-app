(() => {
  ...
  async function api(url, options) {
    try { const res = await fetch(url, options); const data = await res.json(); return res.ok ? data : { error: data.error || "OcurriÃ³ un error" }; }
    catch (_) { return { error: "No hay conexiÃ³n. ProbÃ¡ nuevamente." }; }
  }
  ...
  $("#loginBtn").onclick = async () => {
    const key = $("#keyInput").value.trim(); if (!key) return message($("#loginMsg"), "IngresÃ¡ la palabra clave.", "error");
  ...
  function useSearch(input, msg, callback) { const id = normalizeCarpa(input.value); if (!id) return message(msg, "IngresÃ¡ un nÃºmero entre 1 y 180.", "error"); message(msg, ""); callback(id); }
  ...
  $("#sendReportBtn").onclick = async () => {
    ...
    btn.disabled = true; btn.textContent = "Enviandoâ€¦"; const data = await api(`/api/carpas/${state.carpaId}/reportes`, json("POST", body)); btn.disabled = false; btn.textContent = "Enviar al taller";
    if (data.error) return message($("#atpMsg"), data.error, "error");
    $("#atpView").innerHTML = `<div class="panel success-card"><span class="check">âœ“</span><h2>Reporte enviado</h2><p>El taller ya puede ver el parte de <strong>${data.carpaId}</strong> en su lista de pendientes.</p><button class="primary" id="finishBtn">Finalizar</button></div>`;
  ...
  async function loadPending() {
    $("#pendingList").innerHTML = '<div class="empty">Cargando pendientesâ€¦</div>';
  ...
  async function loadHistory(id) {
    state.historyCarpa = id; selectTab("history"); $("#historyTitle").classList.remove("hidden"); $("#historyTitle strong").textContent = id; $("#historyList").innerHTML = '<div class="empty">Cargando historialâ€¦</div>';
    ...
    renderReports($("#historyList"), data.reportes, `${id} todavÃ­a no tiene reportes.`);
  }
  function renderReports(container, reports, empty) {
    ...
    reports.forEach(report => {
      ...
      const tags = [...(report.problemas || [])]; if (report.prioridad === "urgente") tags.unshift("âš  urgente"); $(".tags", node).innerHTML = tags.map(t => `<span class="tag">${escapeHtml(t)}</span>`).join("");
      $(".detail", node).textContent = report.detalle || "Sin observaciones."; $(".zones", node).textContent = `Zonas: ${(report.puntos || report.partes || []).map(pointLabel).join(", ") || "sin marcar"}${report.reportadoPor ? ` Â· ReportÃ³: ${report.reportadoPor}` : ""}`;
